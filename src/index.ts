...
async function handleTool(name: string, args: Record<string, any>): Promise<{ content: Array<{ type: string; text: string }> }> {
  try {
    switch (name) {
      // ==================== Notebook / 笔记本 ====================
      case 'list_notebooks': {
        const result = await api.listNotebooks() as { notebooks: Array<{id:string; name:string; icon:string; closed:boolean}> };
        const text = result.notebooks.map(n =>
          `[${n.id}] ${n.name} ${n.icon ? `(icon: ${n.icon})` : ''} ${n.closed ? '(closed)' : ''}`
        ).join('\n');
      ...
      case 'create_notebook': {
        const result = await api.createNotebook(args.name) as any;
      ...
      case 'get_notebook_conf': {
        const conf = await api.getNotebookConf(args.notebook) as any;
      ...
      // ==================== Block / 块 ====================
      case 'insert_block': {
        const result = await api.insertBlock(args.dataType, args.data, args.parentID, args.prevID) as any;
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
      }

      case 'prepend_block': {
        const result = await api.prependBlock(args.dataType, args.data, args.parentID) as any;
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
      }

      case 'append_block': {
        const result = await api.appendBlock(args.dataType, args.data, args.parentID) as any;
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
      }

      case 'update_block': {
        const result = await api.updateBlock(args.id, args.dataType, args.data) as any;
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
      }
      ...
      case 'check_siyuan_status': {
        ...
        const status = [];
        if (version) status.push('✅ System version API');
        if (sqlTest) status.push('✅ SQL query API');
        if (notebooks) status.push(`✅ Notebook list API (${(notebooks as any[]).length} notebooks)`);
        if (errors.length > 0) {
          status.push('');
  ...
  return {
    tools: Object.entries(toolSchemas).map(([name, schema]) => ({
      ...
      description: typeof schema.description === 'object'
        ? (schema.description as any).zh  // default to Chinese
        : schema.description,
